...
The first thing we need to do this is to read in the .csv file containing our dataset using the read_csv function.
...
data <- read_csv("alzheimers_dataset.csv")
...
So far, we have been using the default ggplot 'settings' for our visualizations. Can we improve the visualizations? Here are some ideas: change the aesthetics of the plot, add labels to barcharts, change y-axis tick mark intervals and limits, add better descriptions to axes.

```{r}
ggplot(categorical_data, aes(SES)) +
  # plot a bar graph and fill bar graphs with blue color
  geom_bar(fill = "blue") +
...
3.  Changed the aesthetics of the graph for a more minimal design
...
ggplot(data, aes(x=eTIV, y=nWBV, color= CDR)) +
  ...
  labs(x = "Estimated total intracranial volume (eTIV)", y="Normalised whole brain volume (nWBV)",color = "Clinical Dementia Rating\n (CDR)") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        panel.background = element_blank(),
...
Now, use the code above and plot your own multidimensional plot to examine how nWBV changes with age (e.g. change 'nWBV' to 'Age'). What is the relationship between nWBV, Age and CDR?
