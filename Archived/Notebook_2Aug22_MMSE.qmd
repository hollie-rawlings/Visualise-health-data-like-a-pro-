...
```{r}
ggplot(data_clean, aes(x=MMSE, y=eTIV, color= CDR)) +
  geom_point() 
```
...
ggplot(data_clean, aes(x=MMSE, y=eTIV, color= CDR)) +
  ...
  labs(x = "Mini-Mental State Examination score (MMSE)", y="Estimated total intracranial volume (eTIV)",color = "Clinical Dementia Rating\n (CDR)") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        panel.background = element_blank(),
...
#@Andrew I think thats a good idea 
```
